...
namespace VirusDetector.FileClassifier
{
    ...
    /// threshold value and then applies <see cref="ActivationNeuron.ActivationFunction">activation function</see>.
    ...
    public class LkActivationNeuron : ActivationNeuron
    {
        ...
        public override double Compute(double[] input)
        {
            ...
            // initial sum value
            double sum = weights.Select((t, i) => t*input[i]).Sum();

            // compute weighted sum of inputs
            sum += threshold;

            // local variable to avoid mutlithreaded conflicts
            double compute = function.Function(sum);
